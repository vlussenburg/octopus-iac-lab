...
| [`teams.tf`](teams.tf) | Two demo users (`developer`, `prod-deployer`) + two scoped teams (`developers`, `prod-deployers`). Multiple `scoped_user_role` bindings each, composing from Octopus's built-in roles (Project viewer + Environment viewer + Deployment creator [Dev-scoped for devs] + Release creator). |
