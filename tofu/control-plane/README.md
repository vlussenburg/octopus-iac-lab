...
| [`tenants.tf`](tenants.tf) | Three tenants (`acme-corp`, `globex`, `initech`) + tag sets (`tier/{S,M,L}`, `mood/{comedy,silicon-valley,stoic}`, `app/randomquotes`) + per-tenant variables (brand colour/icon, mood, replicas) |
