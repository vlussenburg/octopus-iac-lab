...
Policy *content* (YAML files Platform Hub reads) is committed under `.octopus/` — not managed here.

## Optional

Gated by `OCTOPUS_PLATFORM_HUB_ENABLED` (default `true` in the Makefile). Set to `false` in `.env` if your target Octopus doesn't have Platform Hub (older self-host or some SaaS instances). `make apply` skips this stack and the rest still works.
