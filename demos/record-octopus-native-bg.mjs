#!/usr/bin/env node
// Records a single-tab WebM of the Octopus native blue-green demo (PR #28):
//
// Sequence:
...
const NS           = "randomquotes-local-octopus-native-bg-acme-corp-production";
const APP_URL      = "http://local-octopus-native-bg-acme-corp-production.localtest.me:8080/";
...
(async () => {
  ...
  try {
    ...
    await saveRecording(page, "octopus-native-bg");
    await ctx.close();
    await browser.close();
    ttyd.stop();
  }
