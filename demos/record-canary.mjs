#!/usr/bin/env node
// Records a single-tab WebM of the canary demo (PR #28):
//
...
<script>
  ...
  const server = http.createServer((req, res) => {
    ...
    if (req.url === "/kubectl") {
      ...
      // it against the ReplicaSet hashes — that's where the canary
...
(async () => {
  ...
  try {
    ...
    // ----- Scene 3: trigger deploy + watch task to Success -----
    ...
    // ----- Scene 4: kubectl during canary — show steps + weights -----
    // Argo Rollouts canary spec: setWeight 20 → 40 → 60 → 80 → 100,
    // with 20s pauses between steps. Total run ≈ 80–100s. The kubectl
    // panel shows the canary ReplicaSet's replica count climbing as
    // the stable one tapers.
  ...
  const dst = path.join(OUT, "canary.mp4");
